...
CREATE POLICY "Users can upload avatars to their own folder." ON storage.objects FOR INSERT TO public WITH CHECK (bucket_id = 'avatars' AND (auth.uid())::text = (storage.foldername(name))[1]);
DROP POLICY IF EXISTS "Users can update avatars in their own folder." ON storage.objects;
CREATE POLICY "Users can update avatars in their own folder." ON storage.objects FOR UPDATE TO public USING (bucket_id = 'avatars' AND (auth.uid())::text = (storage.foldername(name))[1]);
DROP POLICY IF EXISTS "Users can delete avatars in their own folder." ON storage.objects;
CREATE POLICY "Users can delete avatars in their own folder." ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'avatars' AND (auth.uid())::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Public Access to room_covers" ON storage.objects;
CREATE POLICY "Public Access to room_covers" ON storage.objects FOR SELECT TO public USING (bucket_id = 'room_covers');
DROP POLICY IF EXISTS "Users can upload their own room covers" ON storage.objects;
CREATE POLICY "Users can upload their own room covers" ON storage.objects FOR INSERT TO public WITH CHECK ((bucket_id = 'room_covers') AND (auth.role() = 'authenticated') AND ((storage.foldername(name))[1] = (auth.uid())::text));
