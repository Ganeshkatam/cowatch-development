...
export function verifyRoomInviteCredential(
  roomId: string,
  credential: string,
): boolean {
  try {
    ...
    ) as RoomInviteCredentialPayload;
...
export async function createRoomInvite(
  ...
  if (!room.isPermanent && room.expiresAt && new Date(room.expiresAt).getTime() <= Date.now()) {
    throw new Error("Room is expired");
  }
  ...
  const result = await postgres.query(
    `INSERT INTO room_invites (room_id, token_hash, created_by, expires_at)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
  ...
  return {
    inviteToken,
    expiresAt,
    inviteId: result.rows[0].id as string,
  };
}
...
export async function redeemRoomInvite(
  roomId: string,
  token: string,
): Promise<{ inviteCredential: string; expiresAt: Date }> {
  if (!postgres) throw new Error("Database unavailable");
  ...
  try {
    ...
    const inviteResult = await client.query(
      `SELECT id, expires_at, revoked_at, max_uses, uses
       FROM room_invites
    ...
    const roomResult = await client.query(
      `SELECT status, "isPermanent", "expiresAt"
       FROM rooms WHERE "roomId" = $1`,
      [roomId],
    );
    const room = roomResult.rows[0];
